...
pool.promise().getConnection()
  .then(connection => {
    console.log("Successfully connected to the database pool with SSL/CA verification.");
    connection.release(); // Release the connection back to the pool
  })
  .catch(err => {
    console.error("Failed to connect to the database pool with SSL/CA verification:", err);
  });